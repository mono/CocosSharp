...
// the node pool.
int CreateProxy(b2AABB& aabb, void* userData)
{
...
bool MoveProxy(int proxyId, b2AABB& aabb, b2Vec2 displacement)
{
    Debug.Assert(0 <= proxyId && proxyId < m_nodeCapacity);
...
void InsertLeaf(int leaf)
{
    ...
    while (index != b2_nullNode)
    {
        ...
        Debug.Assert(child1 != b2_nullNode);
        Debug.Assert(child2 != b2_nullNode);
...
void ValidateStructure(int index) const
{
    ...
    if (index == m_root)
    {
        Debug.Assert(m_nodes[index].parent == b2_nullNode);
    }
