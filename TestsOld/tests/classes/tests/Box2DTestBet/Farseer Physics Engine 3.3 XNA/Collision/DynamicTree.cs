...
namespace FarseerPhysics.Collision
{
    ...
    {
        private static RawList<DynamicTreeNode<T>> _unused = new RawList<DynamicTreeNode<T>>();
        ...
        internal DynamicTreeNode<T> Child1;
        internal DynamicTreeNode<T> Child2;

        internal int LeafCount;
        internal DynamicTreeNode<T> ParentOrNext;
        internal T UserData;
        ...
        public static DynamicTreeNode<T> Create()
        {
            DynamicTreeNode<T> ret;
            if (_unused.count > 0)
            {
                ret = _unused.Elements[_unused.count - 1];
                _unused.count--;
            }
            else
            {
                ret = new DynamicTreeNode<T>();
            }
            return ret;
        }
    ...
    public class DynamicTree<T>
    {
        ...
        private static Stack<DynamicTreeNode<T>> _stack = new Stack<DynamicTreeNode<T>>(256);
        private DynamicTreeNode<T> _freeList;
        ...
        private DynamicTreeNode<T> _root;
        ...
        public DynamicTree()
        {
            ...
            {
                _nodes[i] = DynamicTreeNode<T>.Create();
                _nodes[i].Id = i;
            }
            
            for (int i = 0; i < _nodeCapacity - 1; ++i)
            {
                _nodes[i].ParentOrNext = _nodes[i + 1];
            }

            _nodes[_nodeCapacity - 1].ParentOrNext = null;
        ...
        public int AddProxy(ref AABB aabb, T userData)
        {
            ...
            proxy.AABB.LowerBound = aabb.LowerBound - r;
            proxy.AABB.UpperBound = aabb.UpperBound + r;
            proxy.AABB.UpdatePerimeter();
        ...
        private void RemoveProxy(DynamicTreeNode<T> proxy)
        {
            //Debug.Assert(0 <= proxyId && proxyId < _nodeCapacity);
            Debug.Assert(proxy.Child1 == null);
        ...
        {
            Debug.Assert(0 <= proxyId && proxyId < _nodeCapacity);
            return MoveProxy(_nodes[proxyId], ref aabb, displacement);
        }
        ...
        public bool MoveProxy(DynamicTreeNode<T> proxy, ref AABB aabb, Vector2 displacement)
        {
            Debug.Assert(proxy.Child1 == null);

            if (proxy.AABB.Contains(ref aabb))
            {
                return false;
            }
        ...
        public void Rebalance(int iterations)
        {
            ...
            for (int i = 0; i < iterations; ++i)
            {
                ...
                while (node.Child1 != null)
                {
                    ...
                    node = (selector == 0) ? node.Child1 : node.Child2;
        ...
        {
            var _stack = DynamicTree<T>._stack;
        ...
        public void RayCast(Func<RayCastInput, int, float> callback, ref RayCastInput input)
        {
            ...
            while (_stack.Count > 0)
            {
                ...
                if (node.Child1 == null)
                {
                    ...
                    float value = callback(subInput, node.Id);
        ...
        private int CountLeaves(DynamicTreeNode<T> node)
        {
            if (node == null)
            {
                return 0;
        ...
        private DynamicTreeNode<T> AllocateNode()
        {
            ...
            if (_freeList == null)
            {
                ...
                _nodes[_nodeCapacity - 1].ParentOrNext = null;
                _freeList = _nodes[_nodeCount];
            }

            // Peel a node off the free list.
            var node = _freeList;
            
            _freeList = node.ParentOrNext;
            
            node.ParentOrNext = null;
            node.Child1 = null;
            node.Child2 = null;
            node.LeafCount = 0;
        ...
        private void FreeNode(DynamicTreeNode<T> node)
        {
            //Debug.Assert(0 <= nodeId && nodeId < _nodeCapacity);
            //Debug.Assert(0 < _nodeCount);
            node.ParentOrNext = _freeList;
            _freeList = node;
            --_nodeCount;
        }

        private void InsertLeaf(DynamicTreeNode<T> leaf)
        {
            ++_insertionCount;
            ...
            while (sibling.Child1 != null)
            {
                ...
                sibling.AABB.Combine(ref leafAABB);
                sibling.LeafCount += 1;

                float siblingArea = sibling.AABB.Perimeter;
                AABB parentAABB = new AABB();
                parentAABB.Combine(ref sibling.AABB, ref leafAABB);
                ...
                {
                    AABB aabb = new AABB();
                    aabb.Combine(ref leafAABB, ref child1.AABB);
                ...
                {
                    AABB aabb = new AABB();
                    aabb.Combine(ref leafAABB, ref child2.AABB);
            ...
            var oldParent = sibling.ParentOrNext;
            var newParent = AllocateNode();
            
            newParent.ParentOrNext = oldParent;
            newParent.UserData = default(T);
            newParent.AABB.Combine(ref leafAABB, ref sibling.AABB);
            newParent.LeafCount = sibling.LeafCount + 1;
            ...
            if (oldParent != null)
            {
                ...
                sibling.ParentOrNext = newParent;
                leaf.ParentOrNext = newParent;
            }
            else
            {
        ...
        private void RemoveLeaf(DynamicTreeNode<T> leaf)
        {
            if (leaf == _root)
            {
                _root = null;
            ...
            var parent = leaf.ParentOrNext;
            var grandParent = parent.ParentOrNext;
            DynamicTreeNode<T> sibling;
            ...
            if (grandParent != null)
            {
                ...
                sibling.ParentOrNext = grandParent;
                FreeNode(parent);
                ...
                while (parent != null)
                {
                    parent.AABB.Combine(ref parent.Child1.AABB,
                                                ref parent.Child2.AABB);

                    Debug.Assert(parent.LeafCount > 0);
                    parent.LeafCount -= 1;

                    parent = parent.ParentOrNext;
                }
            }
            else
            {
                _root = sibling;
                sibling.ParentOrNext = null;
                FreeNode(parent);
            }
        }

        private int ComputeHeight(DynamicTreeNode<T> node)
        {
            if (node == null)
            {
                return 0;
...
namespace FarseerPhysics.Collision
{
    ...
    public class DynamicTree<T>
    {
        ...
        public void Rebalance(int iteration)
        {
            if (_root != NullNode)
            {
