...
namespace Cocos2D
{
    ...
    public class CCTimer : SelectorProtocol
    {
        private CCScheduler _scheduler;
        private readonly SelectorProtocol Target;
        ...
        public CCTimer(CCScheduler scheduler, SelectorProtocol target, SEL_SCHEDULE selector)
...
public class CCScheduler
{
    ...
    private readonly Dictionary<SelectorProtocol, HashSelectorEntry> m_pHashForSelectors =
        new Dictionary<SelectorProtocol, HashSelectorEntry>();

    private readonly Dictionary<SelectorProtocol, HashUpdateEntry> m_pHashForUpdates =
        new Dictionary<SelectorProtocol, HashUpdateEntry>();
    ...
    private static SelectorProtocol[] s_pTmpSelectorArray = new SelectorProtocol[128];
    ...
    */
    ...
    public List<SelectorProtocol> PauseAllTargets()
    {
        return PauseAllTargetsWithMinPriority(int.MinValue);
    }

    public List<SelectorProtocol> PauseAllTargetsWithMinPriority(int minPriority)
    {
        var idsWithSelectors = new List<SelectorProtocol>();
    ...
    public void ResumeTargets(List<SelectorProtocol> targetsToResume)
    {
        foreach (SelectorProtocol target in targetsToResume)
        {
            ResumeTarget(target);
        }
    }
