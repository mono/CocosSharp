	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public int FindEntry(string name, bool ignoreCase)
		{
			...
			for (int i = 0; i < entries_.Length; i++) {
                if (string.Compare(name, entries_[i].Name, CultureInfo.InvariantCulture, ignoreCase ? CompareOptions.OrdinalIgnoreCase : CompareOptions.None) == 0)
                {
					return i;
				}
			}
...
//		}
		...
		public void BeginUpdate(IArchiveStorage archiveStorage, IDynamicDataSource dataSource)
		{
            ...
            updates_.Sort(new Comparison<ZipUpdate>(new UpdateComparer().Compare));
 ...
 */
		...
		{
            throw new ZipException("Compression output not supported");
		}
