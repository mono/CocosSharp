...
namespace FarseerPhysics.Dynamics
{
    ...
    public class World
    {
            ...
            if (_bodyRemoveList.Count > 0)
            {
                foreach (Body body in _bodyRemoveList)
                {
                    ...
                    while (ce != null)
                    {
                        ContactEdge ce0 = ce;
                        ce = ce.Next;
                        ContactManager.Destroy(ce0.Contact, -1);
                    }
        ...
        private void Solve(ref TimeStep step)
        {
            ...
            var bodies = BodyList.Elements;
            for (int i = 0, count = BodyList.count; i < count; i++)
            {
                bodies[i].Flags &= ~BodyFlags.Island;
            }

            var contacts = ContactManager.ContactList.Elements;
            for (int i = 0, count = ContactManager.ContactList.count; i < count; i++)
            {
                contacts[i].Flags &= ~ContactFlags.Island;
            }

            var joins = JointList.Elements;
            for (int i = 0, count = JointList.count; i < count; i++)
            {
                joins[i].IslandFlag = false;
            }
            ...
            {
                Body seed = BodyList.Elements[index];
        ...
        private void SolveTOI(ref TimeStep step)
        {
            ...
            for (; ; )
            {
                ...
                var islandBodies = Island.Bodies;
                for (int i = 0, count = Island.BodyCount; i < count; ++i)
                {
                    Body body = islandBodies[i];
