...
namespace FarseerPhysics.Common
{
    ...
    public class Vertices : CCRawList<Vector2>
    {
        ...
        {
            return this.Elements[NextIndex(index)];
        }
        ...
        {
            return this.Elements[PreviousIndex(index)];
        }
        ...
        public float GetSignedArea()
        {
            ...
            {
                int j = (i + 1) % Count;
                area += elements[i].X * elements[j].Y;
                area -= elements[i].Y * elements[j].X;
            }
            area /= 2.0f;
            return area;
        }
        ...
        public Vector2 GetCentroid()
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                // Triangle vertices.
                Vector2 p1 = pRef;
                Vector2 p2 = elements[i];
                Vector2 p3 = i + 1 < count ? elements[i + 1] : elements[0];
        ...
        public AABB GetCollisionBox()
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                {
                    lowerBound.X = elements[i].X;
                }
                if (this[i].X > upperBound.X)
                {
                    upperBound.X = elements[i].X;
                }

                if (this[i].Y < lowerBound.Y)
                {
                    lowerBound.Y = elements[i].Y;
                }
                if (this[i].Y > upperBound.Y)
                {
                    upperBound.Y = elements[i].Y;
                }
            }

            return new AABB(ref lowerBound, ref upperBound);
        }
        ...
        public void Translate(ref Vector2 vector)
        {
            var elements = this.Elements;
            for (int i = 0; i < count; i++)
                 Vector2.Add(ref elements[i], ref vector, out elements[i]);
        }
        ...
        public void Scale(ref Vector2 value)
        {
            var elements = this.Elements;
            for (int i = 0; i < count; i++)
                Vector2.Multiply(ref elements[i], ref value, out elements[i]);
        }
        ...
        public void Rotate(float value)
        {
            ...
            var elements = this.Elements;
            for (int i = 0; i < count; i++)
                Vector2.Transform(ref elements[i], ref rotationMatrix, out elements[i]);
        }
        ...
        public bool IsConvex()
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                Vector2 edge = elements[i2] - elements[i1];
                ...
                for (int j = 0; j < Count; ++j)
                {
                    ...
                    Vector2 r = elements[j] - elements[i1];
        ...
        public bool IsSimple2()
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                int j = (i + 1) % Count;
                Vector2 iToj = elements[j] - elements[i];
                ...
                int k = startK;
                Vector2 iTok = elements[k] - elements[i];
                bool onLeftSide = Vector2.Dot(iTok, iTojNormal) >= 0;
                Vector2 prevK = elements[k];
                ++k;
                for (; k <= endK; ++k)
                {
                    int modK = k % count;
                    iTok = elements[modK] - elements[i];
                    if (onLeftSide != Vector2.Dot(iTok, iTojNormal) >= 0)
                    {
                        Vector2 prevKtoK = elements[modK] - prevK;
                        Vector2 prevKtoKNormal = new Vector2(prevKtoK.Y, -prevKtoK.X);
                        if ((Vector2.Dot(elements[i] - prevK, prevKtoKNormal) >= 0) !=
                            (Vector2.Dot(elements[j] - prevK, prevKtoKNormal) >= 0))
                        {
                            return true;
                        }
                    }
